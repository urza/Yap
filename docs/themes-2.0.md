...
6. **Is the day wash too pale?** With messages on screen the garden wash is justified and text is
   legible in both modes, but empty rooms read pastel. The bottom-stop alphas (0.58 light, 0.68 dark)
   are one number each in `panel()` — cheap to retune once it has been lived with.
...
### Step 5 — Verify on real screens *(remaining)*
