...
- **Built-in emoji packs**: same thing, but committed — one folder per pack under `wwwroot/emoji-packs/`, shipped to every instance, one picker section each. Server customs win the shared shortcode namespace. Either folder may hold a `keywords.json` (shortcode → space-separated terms) adding picker-search terms beyond the shortcode. See `wwwroot/emoji-packs/README.md`.
