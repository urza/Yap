...
public partial class EmojiService
{
    ...
    public MarkupString ConvertEmojisToTwemoji(string text)
    {
        if (string.IsNullOrEmpty(text))
            return new MarkupString(text);

        // Check if message contains only emojis (and whitespace)
        var isEmojiOnly = IsEmojiOnlyMessage(text);
        var emojiSize = isEmojiOnly ? "3em" : "1.2em";
        var verticalAlign = isEmojiOnly ? "-0.4em" : "-0.2em";
